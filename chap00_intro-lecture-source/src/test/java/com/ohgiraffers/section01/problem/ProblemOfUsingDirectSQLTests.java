...
class ProblemOfUsingDirectSQLTests {
    ...
    void closeConnection() throws SQLException {
        ...
        con.rollback(); // 테스트코드가 데이터베이스에 반영되지 않도록 하기 위한 작업
        con.close();
    }
    ...
    * */
    ...
    void testDirectSelectSql() throws SQLException {
        ...
        List<Menu> menuList = new ArrayList<>(); // 메뉴 리스트 생성
        while (rset.next()){
            Menu menu = new Menu();
    ...
    void testDirectInsertSql() throws SQLException {
        ...
        int result = pstmt.executeUpdate(); // 성공시 1 반환
    ...
    * 이러한 수정 영향을 미치는 것은 오류를 발생시킬 가능성도 있지만 유지보수상에도 악영향을 미친다.
    * 또한 객체를 사용할 때 SQL에 의존하면 객체에 값이 무엇이 들어있는지 확인하기 위해 SQL을 매번 살펴야한다.
    * */

    // 2-1. 조회 항목 변경에 대한 의존성 확인
    ...
    * 3-1. 상속 문제
    * 객체 저장 언어의 상속 개념과 유사한 것이 데이터 베이스의 서브타입엔티티이다.
    ...
    *  public class Menu {
    ...
    // 4.동일성 보장 문제
    @DisplayName("조회한 두 개의 행을 담은 객체를 동일 비교 테스트")
    @Test
    void testEquals() throws SQLException {
        ...
        Assertions.assertEquals(menu, menu2); // failed. 이유? 값은 같지만 주소가 다르기 때문에 / jpa 에서는 success
    }
